...
- **RBAC bound to chain-derived state, not a local role table.** `ProtocolAuthorityGuard` checks the session wallet against the protocol authority as recorded in the chain-derived `protocol` projection — never a role column someone can edit. If that authority rotates on-chain, access follows automatically once the indexer observes it: nothing to update here, no stale admin row to forget. The check is a projection read rather than an RPC per request, which keeps admin calls fast and keeps the API up when the RPC is not; the cost is that rotation lands at indexer latency.
...
**Operator transactions are now built and verified** — see below. The browser-wallet UX has since had its human pass with Phantom too, which closed the last unverified path: sign-in, the authority's admin panel, and a second account's 403 were all confirmed in the browser rather than only replayed headlessly. That pass found two real defects, both fixed — see "Wallet adapter" below.
...
- **Sign-In-With-Solana in the browser.** `useSession` requests a nonce, has the wallet sign it, and trades the signature for a session JWT. The wallet signs a *message*, never a transaction — signing in costs nothing and moves no funds. A stored session is bound to the wallet that created it and discarded when a *different* wallet connects, so switching accounts can't leave you authenticated as the previous one.

- **Two defects the manual pass caught that headless replay could not.** The session was wiped on every page load: the wallet adapter reports `publicKey === null` for a tick before `autoConnect` resolves, and the restore effect read that null as "a different wallet connected" and cleared `localStorage` before the real wallet ever arrived. A null wallet now drops the in-memory session without touching storage. Separately, `/operator` rendered *nothing* for a wallet that hadn't signed in — visually identical to a wallet that signed in and was refused, which are opposite outcomes. The page now states which one happened. Both bugs were invisible to the headless auth tests, which hold a token directly and never mount a wallet adapter: a reminder that testing the transport is not testing the UX.
- **`/operator`** lists the routers the connected wallet owns and renders an admin audit panel only for the on-chain protocol authority. The page just calls the endpoint and reacts to 200 vs 403 — the API resolves "is this the authority" against the chain-derived protocol projection, not a role anyone can set by hand, so it follows an authority rotation automatically (at indexer latency, not instantly).
